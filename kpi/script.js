...
async function syncEmployeesWithRealData() {
  /* ① ХЭН БАЙСАН Ч эхлээд R2-оос — Firestore-ийн 268 уншилт зарцуулахгүй,
     квот дуусах үед ч жагсаалт хоосон болохгүй. */
  var shown = false;
  try {
  ...
  /* ② АДМИН нь эх сурвалжаас (users) шинэчилнэ. Энэ нь ЗААВАЛ биш —
     унавал R2-гийн хуулбар аль хэдийн харагдаж байгаа тул хоосон болохгүй.
     Амжилттай бол R2 дахь хувилбарыг шинэчилж, бусад хүнд хүргэнэ. */
  var real = await buildEmployeesFromRealData();
  if (!real || !real.length) {
    if (shown) {
      console.warn('[emp] Firestore шинэчлэл бүтсэнгүй — R2-гийн хуулбар хэвээр');
      setTimeout(function () {
        toast('📋 Ажилтны жагсаалтыг шинэчилж чадсангүй — хадгалсан хуулбарыг харуулж байна.', 'warn');
      }, 1200);
      return true;
    }
    return false;
...
function _riskNormNameRaw(s) {
  return String(s || '').toLowerCase()
    .replace(/[«»"'`.,\-–—()]/g, ' ')
    /* ⚠ JS-ийн \b нь кирилл үсэгт ажиллахгүй тул «алба, хэлтэс, газар …»
       гэсэн үгс таслагдалгүй үлддэг байв. Үгээр нь салгаж шүүнэ (доор). */
    .replace(/(ийн|ын|ний|ны|гийн|тэй|той)\s*$/g, '')
    .replace(/\s+/g, ' ').trim()
    /* Харьяалахын дагаварыг ҮГ БҮРИЙН төгсгөлөөс таслана (JS-ийн \b нь
       кирилл үсэгт ажиллахгүй тул регексээр биш, үгээр нь боловсруулна) */
    .split(' ').map(function (t) { return t.replace(/(гийн|ийн|ний|ын|ны)$/, ''); }).join(' ')
    /* «ХАБЭА-н ахлах менежер» ↔ «ХАБЭА ахлах менежер» — зураасаар салсан
       харьяалахын нөхцөл бие даасан үг болж үлддэг. JS-ийн \b нь кирилл
       үсэгт ажиллахгүй тул үгээр нь салгаж шүүнэ. */
    .split(' ').filter(function (t) {
      if (!t) return false;
      if (/^(н|ы|ий|ийн|ын|ны|г|гийн)$/.test(t)) return false;              // үлдэгдэл нөхцөл
      if (/^(алба|хэлтэс|нэгж|газар|тасаг|цех|баг)$/.test(t)) return false; // бүтцийн үг
      return true;
    }).join(' ');
}
...
/* ══════════════════════════════════════════════════════════════════════
   АРГА ХЭМЖЭЭНИЙ БИЕЛЭЛТ
   ----------------------------------------------------------------------
   Эрсдэл бүрийн «Авах арга хэмжээ» нүд нь ихэвчлэн ОЛОН арга хэмжээг нэг
   дор бичсэн байдаг. Түүнийг нэг бүрчлэн задалж, хариуцагч бүрд:
     · хугацаа (эрсдлийн ТҮВШНЭЭС хамаарна: A-7, B-14, C-30, D/E-60 хоног)
     · гүйцэтгэлээ БАРИМТААР (зураг/файл) баталгаажуулах
   гэсэн шаардлага тавина. Баримт орсон даруйд БИЕЛСЭНД тооцно.
   Хугацаа хэтэрсэн, аваагүй арга хэмжээг өдөр бүр сануулна.
   ══════════════════════════════════════════════════════════════════════ */
var MEA_PREFIX = 'measures/';
...
function _meaIsAction(s) {
  ...
  /* -х төгсгөлтэй (дарах, солих, болох, шалгах, зүүх, хангах)
     ⚠ «байх, хийх» гэх мэт й+х төгсгөлийг ОРХИГДУУЛЖ байв — харин монгол
     ХАБЭА-д «…хүрэхгүй БАЙХ» гэдэг хамгийн түгээмэл хэлбэр. Тиймээс
     хоёр дүрэм нэг мөр болж наалдаж байсан. */
  if (/(?:а|э|о|ө|и|ы|у|ү|й)х$/.test(last)) return true;
  /* Үгүйсгэсэн хэлбэр: ашиглахгүй, хүрэхгүй, гаргахгүй */
  if (/хгүй$/.test(last)) return true;
  /* -на/-нэ/-но/-нө (шалгана, хөтөлнө) */
  if (/(?:на|нэ|но|нө)$/.test(last)) return true;
  if (/(?:улна|үлнэ|лгана|лгэнэ)$/.test(last)) return true;
  /* «… байх ЁСТОЙ / ШААРДЛАГАТАЙ / ҮҮРЕГТЭЙ» гэсэн тогтцууд */
  if (/^(?:ёстой|ёсгүй|хэрэгтэй|шаардлагатай|үүрэгтэй|эрхтэй|болохгүй|хориглоно|зөвшөөрөгдөнө)$/.test(last)) return true;
  return false;
}
/* ⚠ «…илэрвэл», «…ажиллахдаа», «…авах үед» гэж төгссөн хэсэг нь
   ДАГААГҮЙ — үндсэн заавар нь ДАРААГИЙН хэсэгт байна. Дангаараа үлдвэл
   «Жолооч нойр дутуу шинж тэмдэг илэрвэл» гээд яахыг нь хэлээгүй үлдэнэ. */
function _meaIsPartial(str) {
  var t = String(str || '').trim().replace(/[.;:,]+$/, '');
  if (!t) return false;
  var last = (t.split(/\s+/).pop() || '').toLowerCase();
  if (/(?:вэл|вал|вол|вөл)$/.test(last)) return true;          /* илэрвэл, байвал */
  if (/(?:хдаа|хдээ|хдоо|хдөө)$/.test(last)) return true;  /* ажиллахдаа */
  if (/(?:нгуут|нгүүт)$/.test(last)) return true;
  if (/^(?:үед|үедээ|тохиолдолд|дараа|өмнө|зэргээр|байдлаар|маягаар|хэлбэрээр|вэ|бол)$/.test(last)) return true;
  return false;
}
...
function _riskMeasuresRaw(r) {
  ...
  /* ⭐ 0-р шат: ӨГҮҮЛБЭРИЙН ТӨГСГӨЛ бол ШИНЭ арга хэмжээ.
     «Маск хангах. Соруулга суурилуулах. Рентген.» = 3 ӨӨР ажил байтал
     өмнө нь НЭГ мөр болж үлддэг байв. Цэгийн дараа ТОМ үсгээр эхэлж
     байвал л таслана — «1.5 метр», «FFP2)» зэрэгт хөндөгдөхгүй.
     ⚠ lookbehind ашиглахгүй (хуучин Safari дээр script бүхэлдээ унана). */
  raw = raw.replace(/([а-яөүёa-z0-9\)\]])\.[ \t]+([А-ЯӨҮЁA-Z])/g, '$1.\n$2');
  ...
  out.forEach(function (p) {
    ...
    if (!t) return;
    /* ⚠ «…импакт судлах, боломжтой бол авах» гэснийг тасалж «Боломжтой бол
       авах» гэсэн өнчин мөр гаргадаг байв — ЮУГ авахыг нь хэлээгүй.
       Хамааралтай үгээр эхэлсэн хэсэг бол өмнөхийнхөө үргэлжлэл мөн. */
    /* Аль нэг зүйлийг заасан үг (түүнийг, үүний) — ДАНГААРАА УТГАГҮЙ */
    var depRef = /^(?:түүний|үүний|зэргийг|улмаар|ингэснээр|тэгэхдээ|цаашид|эсвэл|эсхүл|мөн )/i.test(t);
    /* НӨХЦӨЛТ (боломжтой бол…) — ЗӨВХӨН БОГИНО бол үргэлжлэл.
       «Боломжтой бол авах» = ЮУГ авах нь үлдсэн → нийлүүлнэ.
       «Хэрэв засвар хийхээр унтрааж байгаа бол ПАЙЗ зүүх» = бүтэн заавар → тусдаа. */
    var depCond = /^(?:боломжтой бол|боломжоор|шаардлагатай бол|шаардвал|зайлшгүй бол|хэрэв|хэрвээ|түүнчлэн)/i.test(t) &&
      t.split(/\s+/).length <= 4;
    /* ⚠ Өмнөх нь ӨГҮҮЛБЭР ДУУСГАСАН бол энэ нь ШИНЭ заавар — бүү нийлүүл («…шилжүүлнэ., Шаардлагатай бол…» гэж эвдэж байв) */
    var prevEnded = fin.length && /[.!?]\s*$/.test(fin[fin.length - 1]);
    if ((depRef || depCond || t.length < 15) && fin.length && !prevEnded) {
      fin[fin.length - 1] += ', ' + t; return;
    }
    fin.push(t);
  });
  /* 4-р шат: ДАГААГҮЙ хэсэгийг ДАРААГИЙНХТАЙ нь нийлүүлнэ */
  var fin2 = [];
  for (var fi = 0; fi < fin.length; fi++) {
    var cur = fin[fi];
    var guard = 0;
    while (fi < fin.length - 1 && _meaIsPartial(cur) && guard++ < 6) {
      cur = cur.replace(/[,;]+\s*$/, '') + ', ' + fin[fi + 1];
      fi++;
    }
    fin2.push(cur);
  }
  /* Хамгийн сүүлийнх нь ДАГААГҮЙ бол урагш нийлэх зүйл алга — ӨМНӨХТЭЙ нь нийлүүлнэ.
     (Эх файлын нүд дундаа тасарсан тохиолдолд өнчин мөр гаргахгүй.) */
  if (fin2.length > 1 && _meaIsPartial(fin2[fin2.length - 1])) {
    var tail = fin2.pop();
    fin2[fin2.length - 1] = fin2[fin2.length - 1].replace(/[,;]+\s*$/, '') + ', ' + tail;
  }
  fin = fin2;
...
function riskMeasureDue(r) {
  var days = MEA_DAYS[riskLevel(r).code] || 60;
  var base = (r && (r.createdAt || r.at)) ||
    (RISK_INDEX && RISK_INDEX.updatedAt) || new Date().toISOString();
  var d = new Date(base);
  if (isNaN(d)) d = new Date();
  d.setDate(d.getDate() + days);
  return _ymd(d);
}
function meaDaysLeft(due) {
  if (!due) return null;
  var d = new Date(due + 'T23:59:59'), n = new Date();
  return Math.ceil((d - n) / 86400000);
}
/* Энэ арга хэмжээг ХЭН авах ёстой вэ — файлын «Хэн хяналт тавих» баганаар */
function riskMeasureOwners(r) {
  var own = riskResolveOwners(r);
  return { emps: own.emps, unresolved: own.unresolved,
    label: String((r && r.responsible) || '').trim() };
}
function riskMeasureIsMine(r, emp) {
  if (!emp) return false;
  var own = riskMeasureOwners(r);
  if (own.emps.some(function (e) { return e.uid === emp.uid || e.id === emp.id; })) return true;
...
/* ══════════════════════════════════════════════════════════════════════
   ...
   ══════════════════════════════════════════════════════════════════════ */
/* Ажилтан ӨӨРӨӨ хийдэг зан үйл, ажлын аргачлал.
   ⚠ «STOP товчлуурыг дарах», «таслуурыг унтраах» зэрэг ТУХАЙН АЖИЛТНЫ
   гар дээрх үйлдлүүд ч энд орно (өмнө нь удирдлага руу явж байсан). */
var MEA_RE_SELF = /ххх|хамгаалах хэрэгсэл|хамгаалалтын хэрэгсэл|бээлий|маск|дуулга|хамгаалалтын шил|нүдний шил|чихэвч|хамгаалах бүс|ажлын хувцас|гутал|өмс|зүү|хэрэглэ|мөрд|баримтла|дага|болгоомж|анхаар|цэвэр байлга|эмх цэгц|ариун цэвэр|гараа угаа|зөв аргаар|зөв байрлал|зөв суу|гарцаар|хурдаа|тайлбал|унтраа|түгж|хориглоно|хориотой|байхгүй байх|хүрэхгүй байх|давахгүй байх|оролдохгүй|ярихгүй|танилцаж|мэдээлэх|мэдэгд|товчлуур|товчийг|товч дар|зогсоосны дараа|салгасны дараа|хүлээж байх|дуудах|тусламж дуудах/i;
...
async function meaLoad(dept, force) {
  var key = meaFileFor(dept);
  if (!force && MEA_STORE[key]) return MEA_STORE[key];
  try {
    var j = await riskR2GetJson(key);
    MEA_STORE[key] = (j && Array.isArray(j.rows)) ? j : { rows: [] };
  } catch (e) { MEA_STORE[key] = MEA_STORE[key] || { rows: [] }; }
  return MEA_STORE[key];
}
async function meaSave(dept, store) {
  var key = meaFileFor(dept);
  MEA_STORE[key] = store;
  return await riskR2PutJson(key, store);
}
...
/* ⭐ Нугалдаг хэсэг — нэрсийн ХАНА (52 хүн) самбарыг 311px болгож сунгадаг
   байсныг нэг мөр болголоо. Дарвал л нээгдэнэ. */
function ackFold(label, inner, bd, n) {
  if (!inner) return '';
  return '<details style="margin-top:9px;padding-top:9px;border-top:1px solid ' + bd + '">' +
    '<summary style="cursor:pointer;font-size:12px;font-weight:700;list-style:none;display:flex;align-items:center;gap:6px">' +
    '<i class="ti ti-chevron-right" style="font-size:14px"></i>' + label +
    (n ? ' <span style="opacity:.6;font-weight:600">(' + n + ')</span>' : '') + '</summary>' +
...
function ackBannerHTML() {
  ...
  if (A.row) {
    return box('#ECFDF5', '#A7F3D0', '#065F46',
      ...
      '</div>' + ackFold('Гарын үсгийн дараалал', chain, '#A7F3D0'));
  }
  if (!A.gate.ok) {
    ...
    return box('#F8FAFC', '#E2E8F0', '#475569',
      ...
      '</div></div>' +
      ackFold('Хэн хүлээгдэж байна', (A.gate.miss && A.gate.miss.length
        ? '<div style="font-size:12px;line-height:1.9">' + A.gate.miss.map(function (e) {
            return esc(e.name || '') + ' <span style="color:#94A3B8">(' + esc(e.pos || e.role || '') + ')</span>';
          }).join(' · ') + '</div>'
        : ''), '#E2E8F0', (A.gate.miss || []).length) +
      ackFold('Гарын үсгийн дараалал', chain, '#E2E8F0'));
  }
  ...
  if (!rd.ok) {
    ...
    return box('#FFFBEB', '#FDE68A', '#92400E',
      ...
      '</div>' + ackFold('Гарын үсгийн дараалал', chain, '#FDE68A'));
  }
  ...
  return box('#EEF2FF', '#C7D2FE', '#3730A3',
    ...
    '</div>' + ackFold('Гарын үсгийн дараалал', chain, '#C7D2FE'));
}
...
var RISK_FILTER = { level: '', q: '', cell: 0, dept: '', mx: '' };
...
function riskDashHTML(list, subtitle) {
  ...
  var shown = list.filter(function (r) {
    ...
    if (RISK_FILTER.dept && String(r.dept || '') !== RISK_FILTER.dept) return false;
  ...
  var rows = (RISK_ROWMODE === 'compact' && page.length) ? riskCompactRowsHTML(page) : page.map(function (r) {
  ...
  return riskMatrixHTML(list) +
    ...
    ((RISK_FILTER.level || RISK_FILTER.cell || RISK_FILTER.q || RISK_FILTER.dept || RISK_FILTER.mx)
    ...
    '</div></div></div>' +
    '<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin:0 0 9px">' +
    '<span style="font-size:13px;font-weight:800;color:#475569">Эрсдлүүд (' +
    (more > 0 ? page.length + ' / ' + shown.length : shown.length) + ')</span>' +
    '<span style="margin-left:auto">' + riskRowModeBtnHTML() + '</span></div>' + rows;
}

/* ══ АДМИН/ТУСЛАХ АДМИНД: алба ба ажилтнаар цэгцэлсэн харагдац ══ */
function riskLevelChips(list) {
  return RISK_LEVELS.slice().reverse().map(function (L) {
    var n = list.filter(function (r) { return riskLevel(r).name === L.name; }).length;
    if (!n) return '';
    return '<span style="display:inline-block;min-width:26px;text-align:center;padding:2px 7px;border-radius:7px;' +
      'background:' + L.bg + ';color:' + L.color + ';font-size:11.5px;font-weight:800;margin-right:4px" title="' + L.name + '">' + n + '</span>';
  }).join('') || '<span style="color:#CBD5E1;font-size:12px">—</span>';
}

/* Ажилтан бүрийн эрсдэлийн ачаалал */
function riskEmpStats(list) {
  var emps = (DB.employees || []).slice();
  if (isDeptHead() && SESSION && SESSION.dept) emps = emps.filter(function (e) { return e.dept === SESSION.dept; });
  return emps.map(function (e) {
    var mine = list.filter(function (r) { return riskAppliesTo(r, e); });
    var top = 0;
    mine.forEach(function (r) { var s = riskScore(r); if (s > top) top = s; });
    return { e: e, n: mine.length, top: top, list: mine };
  }).filter(function (x) { return x.n > 0; })
    .sort(function (a, b) { return (b.top - a.top) || (b.n - a.n); });
}

function riskEmpDetail(empId) {
  var st = riskEmpStats(risksForView()).filter(function (x) { return x.e.id === empId; })[0];
  if (!st) return;
  var rows = st.list.sort(function (a, b) { return riskScore(b) - riskScore(a); }).map(function (r) {
    var L = riskLevel(r);
    return '<div style="display:flex;align-items:center;gap:11px;padding:10px 0;border-bottom:1px solid #F1F5F9;cursor:pointer" data-risk-open="' + esc(r.id) + '">' +
      '<span style="width:34px;text-align:center;font-size:16px;font-weight:900;color:' + L.color + ';font-family:\'Bricolage Grotesque\',sans-serif">' + riskScore(r) + '</span>' +
      '<span style="flex:1;min-width:0"><span style="display:block;font-size:13px;font-weight:600;color:#1E293B">' + esc(r.hazard) + '</span>' +
      (r.process ? '<span style="display:block;font-size:11.5px;color:#94A3B8;margin-top:2px">' + esc(r.process) + '</span>' : '') + '</span>' +
      '<span style="font-size:10.5px;font-weight:800;color:' + L.color + ';background:' + L.bg + ';border-radius:6px;padding:2px 7px;flex-shrink:0">' + L.name + '</span></div>';
  }).join('');
  var L0 = riskLevel(st.top);
  var node = elc('div', 'modal-info',
    '<div style="display:flex;align-items:center;gap:12px;background:' + L0.bg + ';border:1px solid ' + L0.bd + ';border-radius:12px;padding:12px 14px;margin-bottom:12px">' +
    '<div style="text-align:center"><div style="font-size:28px;font-weight:900;color:' + L0.color + ';font-family:\'Bricolage Grotesque\',sans-serif;line-height:1">' + st.n + '</div>' +
    '<div style="font-size:11px;color:' + L0.color + ';font-weight:700">эрсдэл</div></div>' +
    '<div style="flex:1;font-size:12.5px;color:#475569;line-height:1.6">' +
    esc(st.e.dept || '—') + ' · ' + esc(st.e.role || st.e.pos || '—') + '<br>' +
    'Хамгийн өндөр: <b style="color:' + L0.color + '">' + st.top + ' (' + L0.name + ')</b></div></div>' + rows);
  node.addEventListener('click', function (ev) {
    var o = ev.target.closest('[data-risk-open]');
    if (o) { closeModal(); setTimeout(function () { riskOpenDetail(o.getAttribute('data-risk-open')); }, 60); }
  });
...
function riskAdminSectionsHTML(list) {
  ...
  if (false) {
    ...
    list.forEach(function (r) { (byDept[r.dept || 'Тодорхойгүй'] = byDept[r.dept || 'Тодорхойгүй'] || []).push(r); });
    var dnames = Object.keys(byDept).sort(function (a, b) { return byDept[b].length - byDept[a].length; });
    var drows = dnames.map(function (d) {
      var l = byDept[d];
      var mx = 0; l.forEach(function (r) { var s = riskScore(r); if (s > mx) mx = s; });
      var L = riskLevel(mx);
      var emps = (DB.employees || []).filter(function (e) { return e.dept === d; }).length;
      return '<tr class="dash-click" data-risk-dept-f="' + esc(d) + '">' +
        '<td style="text-align:left;font-weight:600">' + esc(d) + '</td>' +
        '<td>' + emps + '</td>' +
        '<td><b>' + l.length + '</b></td>' +
        '<td style="text-align:left">' + riskLevelChips(l) + '</td>' +
        '<td><span style="font-weight:900;color:' + L.color + '">' + mx + '</span></td></tr>';
    }).join('');
    H += dashCard(dashH('🏢 Албадын эрсдэлийн зураглал', 'Мөр дээр дарж тухайн албаны эрсдлүүдийг шүүнэ') +
      '<div style="overflow-x:auto"><table class="dash-tbl"><thead><tr>' +
      '<th style="text-align:left">Алба</th><th>Ажилтан</th><th>Эрсдэл</th>' +
  ...
  list.forEach(function (r) {
    var ps = (r.positions || []);
    if (!ps.length) ps = ['Бүх ажилтан'];
    ps.forEach(function (p) { (byPos[p] = byPos[p] || []).push(r); });
  });
  var pnames = Object.keys(byPos).sort(function (a, b) { return byPos[b].length - byPos[a].length; }).slice(0, 12);
  if (pnames.length) {
    H += dashCard(dashH('🔧 Албан тушаалаар', 'Аль ажлын байр хамгийн олон эрсдэлтэй вэ') +
      pnames.map(function (p) {
        var l = byPos[p], mx = 0;
        l.forEach(function (r) { var s = riskScore(r); if (s > mx) mx = s; });
        var L = riskLevel(mx);
        var w = Math.max(4, Math.round(l.length / byPos[pnames[0]].length * 100));
        return '<div style="display:flex;align-items:center;gap:11px;padding:6px 0">' +
          '<span style="flex:0 0 150px;font-size:12.5px;color:#475569;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">' + esc(p) + '</span>' +
          '<span style="flex:1;height:9px;background:#F1F5F9;border-radius:5px;overflow:hidden">' +
          '<span style="display:block;height:100%;width:' + w + '%;background:' + L.color + ';border-radius:5px"></span></span>' +
          '<span style="flex:0 0 34px;text-align:right;font-size:12.5px;font-weight:800;color:#1E293B">' + l.length + '</span></div>';
      }).join(''), '18px');
  }
  return H;
}

/* Одоо дэлгэц дээр харагдаж байгаа эрсдлийн дараалал — ← → шилжихэд */
function riskDetailSiblings() {
  var out = [];
  try {
    document.querySelectorAll('.page[data-page="hazards"] [data-risk-open]').forEach(function (el) {
      var v = el.getAttribute('data-risk-open');
...
function riskOpenDetail(id) {
  ...
  /* ⭐ Жагсаалт дээрх дараалал — 12 эрсдэлтэй танилцахад цонхыг хааж,
     доошоо гүйлгэж, дараагийнхыг хайх шаардлагагүй болно. */
  var sib = riskDetailSiblings();
  var at = sib.indexOf(String(id));
  var navHTML = (sib.length > 1 && at >= 0)
    ? '<div style="display:flex;align-items:center;gap:9px;margin-bottom:12px">' +
      '<button class="btn btn-sm btn-secondary" data-risk-nav="prev"' + (at <= 0 ? ' disabled' : '') + '>' +
      '<i class="ti ti-chevron-left"></i> Өмнөх</button>' +
      '<div style="flex:1;text-align:center;font-size:12px;font-weight:700;color:#8A94A6">' +
      (at + 1) + ' / ' + sib.length + ' эрсдэл</div>' +
      '<button class="btn btn-sm btn-secondary" data-risk-nav="next"' + (at >= sib.length - 1 ? ' disabled' : '') + '>' +
      'Дараах <i class="ti ti-chevron-right"></i></button></div>'
    : '';

  var node = elc('div', 'modal-info', navHTML + riskDetailHTML(r));
  /* ⭐ ӨРГӨН цонх — 2 багана багтаж, доошоо гүйлгэх нь бараг арилна */
  buildModal('Эрсдэлийн дэлгэрэнгүй', node, { width: 'min(1240px, 96vw)' });
  ...
  var wire = function (host) {
    ...
    host.addEventListener('click', function (ev) {
      var nb = ev.target.closest('[data-risk-nav]');
      if (!nb || nb.disabled) return;
      var k = nb.getAttribute('data-risk-nav');
      var j = at + (k === 'next' ? 1 : -1);
      if (j < 0 || j >= sib.length) return;
      closeModal();
      setTimeout(function () { riskOpenDetail(sib[j]); }, 60);
    });
  };
  wire(node);
  ...
  meaLoad(dept).then(function (st) {
    ...
    var host = node.parentNode ? node : null;
    if (!host) return;
    host.innerHTML = navHTML + riskDetailHTML(r);
    host._riskNavWired = false;
    wire(host);
  }).catch(function (e) { console.error('[measures]', e); });
}

/* ══════════════════════════════════════════════════════════════════════
   ЭРСДЭЛИЙН ДЭЛГЭРЭНГҮЙ — ажилтанд ойлгомжтой байдлаар
   ⚠ Файлд байгаа агуулгыг ХАСАХГҮЙ, ШИНЭ АГУУЛГА ЗОХИОХГҮЙ.
   Гарчиг, бүтэц, өнгө нь минийх; бичвэр бүр эх датагаас ирнэ.
   ══════════════════════════════════════════════════════════════════════ */
function riskFactBox(icon, label, value, tone) {
  if (value === undefined || value === null || value === '') return '';
  return '<div style="display:flex;gap:11px;padding:11px 0;border-bottom:1px solid #F1F5F9">' +
    '<span style="flex:0 0 22px;font-size:15px;line-height:1.4;text-align:center">' + icon + '</span>' +
    '<span style="flex:1;min-width:0">' +
    '<span style="display:block;font-size:11px;font-weight:800;color:#94A3B8;letter-spacing:.4px;text-transform:uppercase">' + label + '</span>' +
    '<span style="display:block;font-size:14px;color:' + (tone || '#1E293B') + ';margin-top:3px;line-height:1.65;white-space:pre-wrap">' + esc(String(value)) + '</span>' +
    '</span></div>';
}
...
function riskMeasureDoModal(r, mi) {
  var me = null; try { me = myEmp(); } catch (e) {}
  if (!me) { toast('Ажилтны бүртгэл олдсонгүй', 'error'); return; }
  var m = riskMeasures(r)[mi]; if (!m) return;
  var node = elc('div');
  node.innerHTML =
    '<div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:12px;padding:12px 14px;margin-bottom:13px">' +
    '<div style="font-size:11.5px;font-weight:800;color:#94A3B8;letter-spacing:.3px">АВАХ ЁСТОЙ АРГА ХЭМЖЭЭ</div>' +
    '<div style="font-size:13.5px;color:#1E293B;margin-top:4px;line-height:1.55">' + esc(m.text) + '</div>' +
    '<div style="font-size:11.5px;color:#64748B;margin-top:6px">Эрсдэл: ' + esc(r.hazard || '') +
    ' · Хугацаа: ' + riskMeasureDue(r) + '</div></div>' +
    '<label style="font-size:12.5px;font-weight:700;color:#334155;display:block;margin-bottom:6px">Юу хийснээ бичнэ үү</label>' +
    '<textarea id="meaTxt" rows="3" placeholder="Жишээ: Шатны бэхэлгээг хананд бэхэлж, 2 боолт нэмж тавьсан." ' +
    'style="width:100%;padding:11px 13px;border:1.5px solid #E2E8F0;border-radius:11px;font-size:13px;' +
    'font-family:inherit;line-height:1.55;resize:vertical"></textarea>' +
    '<div style="margin-top:13px;background:#FFFBEB;border:1.5px solid #FDE68A;border-radius:12px;padding:12px 14px">' +
    '<div style="font-size:12.5px;font-weight:800;color:#92400E">📎 БАРИМТ ЗААВАЛ ХАВСАРГАНА</div>' +
    '<div style="font-size:11.5px;color:#B45309;margin-top:3px;line-height:1.5">' +
    'Зураг эсвэл файл хавсаргасан үед л арга хэмжээ «авсан» гэж тооцогдоно.</div>' +
    '<input type="file" id="meaFiles" multiple accept="image/*,.pdf,.doc,.docx,.xlsx" ' +
    'style="margin-top:9px;font-size:12.5px;width:100%">' +
    '<div id="meaList" style="margin-top:8px;font-size:12px;color:#92400E"></div></div>' +
    '<button class="btn btn-primary" id="meaSave" style="width:100%;margin-top:13px" disabled>' +
    '<i class="ti ti-check"></i> Гүйцэтгэсэн гэж бүртгэх</button>' +
    '<div id="meaSt" style="margin-top:9px;font-size:12.5px;line-height:1.6"></div>';

  buildModal('Арга хэмжээний гүйцэтгэл', node, { width: '540px' });
  var inp = node.querySelector('#meaFiles'), btn = node.querySelector('#meaSave');
  var st = node.querySelector('#meaSt'), listEl = node.querySelector('#meaList');
  var say = function (h, c) { st.innerHTML = '<span style="color:' + (c || '#64748B') + '">' + h + '</span>'; };

  inp.addEventListener('change', function () {
    var fs = Array.prototype.slice.call(inp.files || []);
    btn.disabled = !fs.length;
    listEl.innerHTML = fs.length
      ? fs.map(function (f) { return '✓ ' + esc(f.name) + ' (' + Math.round(f.size / 1024) + ' KB)'; }).join('<br>')
      : '';
    var big = fs.filter(function (f) { return f.size > 12 * 1024 * 1024; });
    if (big.length) { say('⚠️ 12 MB-аас том файл байна: ' + esc(big[0].name), '#B91C1C'); btn.disabled = true; }
  });

  btn.addEventListener('click', async function () {
    var fs = Array.prototype.slice.call(inp.files || []);
    if (!fs.length) { say('⚠️ Баримт хавсаргана уу', '#B91C1C'); return; }
    btn.disabled = true;
    ...
    try {
      for (var i = 0; i < fs.length; i++) {
        ...
        var url = await r2Put(f, key);
        out.push({ name: f.name, size: f.size, url: (typeof url === 'string' ? url : (TASK_R2 + '/' + key)) });
      }
      btn.innerHTML = '<i class="ti ti-loader-2"></i> Хадгалж байна…';
      var res = await meaAdd(me, r, mi, node.querySelector('#meaTxt').value, out);
      if (!res.ok) { say('⚠️ ' + esc(res.error), '#B91C1C'); btn.disabled = false; btn.innerHTML = old; return; }
      MEA_VIEW.store = await meaLoad(riskCanonDept(me.dept) || me.dept, true);
      closeModal();
      toast('✓ Арга хэмжээ авсанд тооцогдлоо', 'success');
      try { renderHazards(); } catch (e) {}
      setTimeout(function () { riskOpenDetail(r.id); }, 200);
    } catch (e) {
      say('⚠️ Илгээж чадсангүй: ' + esc((e && e.message) || e), '#B91C1C');
      btn.disabled = false; btn.innerHTML = old;
    }
  });
}

/* «Таны авсан арга хэмжээ» — баримттай бүртгэлүүд */
function riskMeasureLogHTML(r, me, store) {
  var mine = ((store && store.rows) || []).filter(function (x) {
    return x.riskId === r.id && x.uid === me.uid;
  ...
  return '<div style="margin-top:14px;padding-top:12px;border-top:1px solid #F1F5F9">' +
    '<div style="font-size:12.5px;font-weight:900;color:#334155;letter-spacing:.3px;margin-bottom:8px">📎 ТАНЫ АВСАН АРГА ХЭМЖЭЭ</div>' +
    mine.map(function (x) {
      return '<div style="background:#fff;border:1px solid #E2E8F0;border-radius:12px;padding:11px 13px;margin-bottom:8px">' +
        '<div style="display:flex;gap:10px;align-items:baseline;flex-wrap:wrap">' +
        '<span style="font-size:11.5px;font-weight:800;color:#059669">' + ackDateMn(x.at) + '</span>' +
        '<span style="flex:1;min-width:140px;font-size:13px;color:#1E293B;line-height:1.55">' +
        esc(x.text || x.measure || '') + '</span></div>' +
        ((x.files || []).length
...
/* ══════════════════════════════════════════════════════════════════════
   5×5 ЭРСДЭЛИЙН МАТРИЦ — «хаана нь юу байгаа» гэдгийг НҮДЭЭР харуулна
   Хэвтээ тэнхлэг = Ослын үр дагавар (N), босоо = Магадлал (P).
...
function riskMatrixHTML(rows, sideHTML) {
  ...
  var PL = ['', 'Бараг үгүй', 'Магадгүй', 'Боломжтой', 'Их', 'Бүрэн'];
  var NL = ['', 'Мэдэгдэхүйц\nбиш', 'Бага\nхэмжээний', 'Хүнд\nбиш', 'Хүнд', 'Онц\nхүнд'];
  var H = '<div class="card" style="padding:14px 16px;margin-bottom:12px">' +
    '<div class="risk-mx-wrap">' +
    '<div style="min-width:0">' +
    '<div style="display:flex;align-items:center;gap:9px;flex-wrap:wrap;margin-bottom:1px">' +
    '<div style="font-size:14px;font-weight:800;color:#1E293B">Эрсдэлийн матриц</div>' +
    '<div style="font-size:11.5px;color:#94A3B8">нүд дарвал шүүнэ</div>' +
    (RISK_FILTER.mx ? '<button class="btn btn-sm" data-risk-mx="" style="background:#F1F5F9;color:#475569;border-color:#E2E8F0;padding:3px 9px">' +
      '<i class="ti ti-x"></i> Цуцлах</button>' : '') + '</div>' +
    '<div style="overflow-x:auto;padding-top:4px">' +
    '<table style="border-collapse:separate;border-spacing:4px;font-family:inherit">' +
    '<tr><td></td><td colspan="5" style="text-align:center;font-size:10.5px;font-weight:800;color:#94A3B8;' +
    'letter-spacing:.4px;padding-bottom:2px">ОСЛЫН ҮР ДАГАВАР →</td></tr>';
  ...
  for (var p = 5; p >= 1; p--) {
    ...
    for (var n = 1; n <= 5; n++) {
      ...
      H += '<td>' + (c
        ...
        : '<div style="width:50px;height:34px;border-radius:8px;background:#FBFCFE;border:1.5px dashed #EEF1F7"></div>') +
        '</td>';
    }
    H += '</tr>';
  }
  ...
  for (var n2 = 1; n2 <= 5; n2++) {
    H += '<td style="text-align:center;font-size:9.5px;font-weight:700;color:#B6BECC;line-height:1.25;padding-top:2px;' +
      'white-space:pre-line">' + n2 + '\n' + NL[n2] + '</td>';
  }
  H += '</tr></table></div>' +
    ...
    (un ? '<button data-risk-mx="none" style="border:1.5px solid #E2E8F0;' +
      ...
      (RISK_FILTER.mx === 'none' ? '#fff' : '#64748B') + ';border-radius:7px;padding:3px 9px;cursor:pointer;' +
      'font-family:inherit;font-size:10.5px;font-weight:800">Үнэлгээгүй ' + un + '</button>' : '') +
    '</div></div>' +
    (sideHTML ? '<div style="min-width:0">' + sideHTML + '</div>' : '') +
    '</div></div>';
  return H;
}

/* Матрицын шүүлтийг мөр дээр хэрэглэнэ */
function riskMxPass(r) {
  if (!RISK_FILTER.mx) return true;
  if (RISK_FILTER.mx === 'none') return riskMxCellOf(r) === null;
  return riskMxKey(r) === RISK_FILTER.mx;
}

/* ══════════════════════════════════════════════════════════════════════
   ЭРСДЭЛИЙН ҮНЭЛГЭЭ ЦЭСНИЙ ТАБУУД
   ⚠ «Даалгавар» цэс нь ТУСДАА үйл ажиллагаа (удирдлага гараар даалгавар
   өгөх). Эрсдэлээс гарсан арга хэмжээ нь ЭНД, эрсдлийнхээ хажууд байна.
   ══════════════════════════════════════════════════════════════════════ */
var RISK_TAB = 'risks';
...
function riskTabsHTML(list) {
  ...
  return '<div style="display:flex;gap:6px;flex-wrap:wrap;border-bottom:1.5px solid #E2E8F0;margin-bottom:14px">' +
    tabs.map(function (t) {
      var on = RISK_TAB === t.key;
      return '<button data-risk-tab="' + t.key + '" style="border:none;background:none;cursor:pointer;font-family:inherit;' +
        'padding:9px 14px 10px;font-size:13.5px;font-weight:' + (on ? '800' : '600') + ';' +
        'color:' + (on ? '#4F46E5' : '#64748B') + ';border-bottom:2.5px solid ' + (on ? '#4F46E5' : 'transparent') + ';' +
        'margin-bottom:-1.5px;display:inline-flex;align-items:center;gap:7px">' +
        '<i class="ti ' + t.icon + '" style="font-size:16px"></i>' + t.label +
        (t.n ? '<span style="background:' + (t.tone || (on ? '#4F46E5' : '#CBD5E1')) + ';color:#fff;border-radius:20px;' +
          'padding:1px 8px;font-size:11px;font-weight:800">' + t.n + '</span>' : '') +
        '</button>';
    }).join('') + '</div>';
}

function riskSectionHead(t, sub) {
  return '<div style="margin:18px 0 2px">' +
    '<div style="font-size:12.5px;font-weight:900;color:#334155;letter-spacing:.3px">' + t + '</div>' +
    (sub ? '<div style="font-size:11.5px;color:#94A3B8;margin-top:1px">' + sub + '</div>' : '') + '</div>';
}
/* Гишүүдийн өгсөн оноог жижиг тэмдэглэгээгээр */
function riskScoreChips(vals, avgVal, label) {
  var v = (vals || []).filter(function (x) { return x > 0; });
  var chips = v.map(function (x) {
    return '<span style="display:inline-block;min-width:20px;text-align:center;background:#F1F5F9;color:#475569;' +
      'border-radius:5px;padding:1px 5px;font-size:11px;font-weight:700;margin-right:3px">' + x + '</span>';
  }).join('');
  return '<div style="display:flex;align-items:center;gap:9px;padding:7px 0">' +
    '<span style="flex:0 0 118px;font-size:12px;color:#64748B">' + label + '</span>' +
    '<span style="flex:1;min-width:0">' + (chips || '<span style="color:#CBD5E1;font-size:12px">—</span>') + '</span>' +
    '<span style="flex:0 0 46px;text-align:right;font-size:13.5px;font-weight:800;color:#1E293B">' +
...
function riskEmpBriefHTML(list, myPos, myDept) {
  ...
  /* ── Хураангуй — матрицын ХАЖУУД орно (хоосон зайг зөв ашиглана) ── */
  var side = '<div style="display:flex;align-items:baseline;gap:15px;flex-wrap:wrap">' +
    '<div><span style="font-size:30px;font-weight:900;color:#312E81;line-height:1;font-family:\'Bricolage Grotesque\',sans-serif">' + all.length + '</span>' +
    '<span style="font-size:12px;color:#4338CA;font-weight:700;margin-left:5px">эрсдэл</span></div>' +
    (high ? '<div><span style="font-size:30px;font-weight:900;color:#DC2626;line-height:1;font-family:\'Bricolage Grotesque\',sans-serif">' + high + '</span>' +
  ...
  side += '<div style="display:flex;gap:7px;flex-wrap:wrap;margin-top:12px">' + chips + '</div>' +
    '<div style="display:flex;gap:8px;margin-top:10px;flex-wrap:wrap">' +
    '<input type="text" id="riskSearch" placeholder="Аюул, байршил, арга хэмжээгээр хайх…" value="' + esc(RISK_FILTER.q || '') + '" ' +
    'style="flex:1;min-width:170px;padding:9px 12px;border:1.5px solid #E2E8F0;border-radius:10px;font-size:13px;font-family:inherit">' +
    ((RISK_FILTER.level || RISK_FILTER.q || RISK_FILTER.mx)
  ...
  /* ⭐ 5×5 МАТРИЦ + хураангуй — НЭГ картад, хажуу хажууд */
  H += riskMatrixHTML(all, side);
  ...
  H += '<div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;margin-bottom:8px">' +
    '<span style="font-size:12px;font-weight:800;color:#94A3B8">' +
    (rows.length === all.length ? 'ЭРСДЭЛ БҮГД' : 'ШҮҮСЭН') + ' · ' + rows.length +
    ' <span style="font-weight:600;color:#CBD5E1">— мөр дээр дарвал дэлгэрэнгүй</span></span>' +
    '<span style="margin-left:auto">' + riskRowModeBtnHTML() + '</span></div>';
  return H + riskEmpRowsHTML(rows);
}

/* ══════════════════════════════════════════════════════════════════════
   НЯГТ ХҮСНЭГТ — нэг эрсдэл = НЭГ МӨР (44px)
   12 эрсдэл 1,190px байснаа ~530px болж НЭГ ДЭЛГЭЦЭНД бүтнээр харагдана.
   «Карт» товчоор дэлгэрэнгүй хэлбэр рүү сэлгэнэ (сонголт нь хадгалагдана).
   ══════════════════════════════════════════════════════════════════════ */
var RISK_ROWMODE = (function () {
  try { return localStorage.getItem('kpi_risk_rowmode') || 'compact'; } catch (e) { return 'compact'; }
})();
function riskRowModeSet(m) {
  RISK_ROWMODE = m;
  try { localStorage.setItem('kpi_risk_rowmode', m); } catch (e) {}
}
function riskRowModeBtnHTML() {
  var b = function (m, ic, lab) {
    var on = RISK_ROWMODE === m;
    return '<button data-risk-rowmode="' + m + '" style="border:none;background:' + (on ? '#fff' : 'transparent') +
      ';box-shadow:' + (on ? '0 1px 3px rgba(15,23,42,.12)' : 'none') + ';border-radius:8px;padding:5px 11px;' +
      'cursor:pointer;font-family:inherit;font-size:12px;font-weight:' + (on ? '800' : '600') +
      ';color:' + (on ? '#4F46E5' : '#8A94A6') + ';display:inline-flex;align-items:center;gap:5px">' +
      '<i class="ti ' + ic + '" style="font-size:14px"></i>' + lab + '</button>';
  };
  return '<span style="display:inline-flex;gap:2px;background:#F1F5F9;border-radius:10px;padding:3px">' +
    b('compact', 'ti-list', 'Хүснэгт') + b('card', 'ti-layout-cards', 'Карт') + '</span>';
}

/* Нэг мөрт хэлбэр */
function riskCompactRowsHTML(rows) {
  var H = '<div style="border:1px solid #E2E8F0;border-radius:12px;overflow:hidden;background:#fff">' +
    '<div style="display:flex;align-items:center;gap:10px;padding:7px 12px;background:#F8FAFC;' +
    'border-bottom:1px solid #E2E8F0;font-size:10.5px;font-weight:800;color:#94A3B8;letter-spacing:.4px">' +
    '<span style="flex:0 0 26px">#</span>' +
    '<span style="flex:0 0 62px">ТҮВШИН</span>' +
    '<span style="flex:2;min-width:0">АЮУЛ</span>' +
    '<span style="flex:3;min-width:0" class="rk-hide-sm">ЮУ ХИЙЖ СЭРГИЙЛЭХ</span>' +
    '<span style="flex:0 0 20px"></span></div>';
  var num = 0;
  rows.forEach(function (r) {
    num++;
    var L = riskLevel(r), sc = riskScore(r), un = riskIsUnscored(r);
    var act = String(r.actions || '').replace(/\s+/g, ' ').trim();
    H += '<div class="risk-row rk-line" data-risk-open="' + esc(r.id) + '" ' +
      'style="display:flex;align-items:center;gap:10px;padding:9px 12px;cursor:pointer;' +
      'border-bottom:1px solid #F4F6FA;border-left:3px solid ' + L.color + '">' +
      '<span style="flex:0 0 26px;font-size:11.5px;font-weight:800;color:#B6BECC">' + num + '</span>' +
      '<span style="flex:0 0 62px"><span style="background:' + L.bg + ';color:' + L.color +
      ';border-radius:6px;padding:2px 7px;font-size:10.5px;font-weight:900;white-space:nowrap">' +
      (un ? '—' : L.code + ' ' + sc) + '</span></span>' +
      '<span style="flex:2;min-width:0;font-size:13px;font-weight:700;color:#1E293B;' +
      'overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="' + esc(r.hazard || '') + '">' +
      esc(r.hazard || '—') + '</span>' +
      '<span class="rk-hide-sm" style="flex:3;min-width:0;font-size:12.5px;color:#64748B;' +
      'overflow:hidden;text-overflow:ellipsis;white-space:nowrap" title="' + esc(act) + '">' +
      (act ? '🛡️ ' + esc(act) : '<span style="color:#CBD5E1">—</span>') + '</span>' +
      '<i class="ti ti-chevron-right" style="flex:0 0 20px;color:#CBD5E1;font-size:16px"></i></div>';
  });
  return H + '</div>';
}
...
function riskEmpRowsHTML(rows) {
  if (RISK_ROWMODE === 'compact') return riskCompactRowsHTML(rows);
  return riskEmpCardsHTML(rows);
}

function riskEmpCardsHTML(rows) {
  var H = '', num = 0;
  rows.forEach(function (r) {
    num++;
    var L = riskLevel(r), sc = riskScore(r), un = riskIsUnscored(r);
...
/* Тод гарчиг — өнгөт зүүн зураастай, талбайн нэрээс ИЛТ өөр */
function riskBigHead(num, title, sub, color) {
...
function riskDetailHTML(r) {
  ...
  var after = riskScoreAfter(r);
  var LA = after > 0 ? riskLevel(after) : null;
  var drop = (after > 0 && sc > 0) ? Math.round((1 - after / sc) * 100) : 0;

  /* Хийх ажлын тоо — чипэнд шууд харуулна */
  var mCnt = 0, mDone = 0, mRule = 0;
  try {
    var store = MEA_VIEW.store, dept0 = riskCanonDept(r.dept) || r.dept || '';
    riskMeasures(r).forEach(function (m) {
      if (m.role === 'self') { mRule++; return; }
      mCnt++;
      if (store && meaDone(store, meaKeyOf(dept0, m.text))) mDone++;
    });
  } catch (e) {}

  /* ══ ТОЛГОЙ — юу вэ, хэр ноцтой вэ (нэг харцаар) ══ */
  var H = '<div style="background:linear-gradient(135deg,' + L.bg + ',#fff);border:1.5px solid ' + L.bd +
    ';border-radius:15px;padding:16px 17px;margin-bottom:13px">' +
    '<div style="display:flex;align-items:flex-start;gap:16px;flex-wrap:wrap">' +
    '<div style="text-align:center;flex:0 0 74px">' +
    '<div style="font-size:34px;font-weight:900;color:' + L.color + ';font-family:\'Bricolage Grotesque\',sans-serif;line-height:1">' +
    (un ? '—' : sc) + '</div>' +
    '<div style="display:inline-block;margin-top:6px;background:' + L.color + ';color:#fff;border-radius:7px;' +
    'padding:2px 9px;font-size:11px;font-weight:900">' + (un ? 'ҮНЭЛГЭЭГҮЙ' : L.code) + '</div></div>' +
    '<div style="flex:1;min-width:230px">' +
    '<div style="font-size:17px;font-weight:800;color:#0F172A;line-height:1.4">' + esc(r.hazard || 'Нэргүй эрсдэл') + '</div>' +
    '<div style="font-size:12px;color:#64748B;margin-top:6px;line-height:1.6">' +
    (pos ? '👤 ' + esc(pos) : '') + (pos && r.dept ? ' &nbsp;·&nbsp; ' : '') +
    (r.dept ? '🏢 ' + esc(r.dept) : '') + '</div></div></div>' +
    /* ── Хураангуй чипс ── */
    '<div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:13px">' +
    riskChip('ЭРСДЭЛИЙН ЗЭРЭГ', un ? 'Тодорхойгүй' : esc(L.name), L.color, L.bg) +
  ...
  /* ══ 2 БАГАНА: зүүн = ЮУ ВЭ, баруун = ЮУ ХИЙХ ══ */
  ...
  /* ── Онооны математик — НУГАЛААС дотор (ажилтанд хэрэггүй, админд хэрэгтэй) ── */
  var hasBreak = (r.pv && r.pv.length) || (r.nv && r.nv.length) || (r.hv && r.hv.length);
  var brk2 = r.rAfterOk && ((r.p2v && r.p2v.length) || (r.n2v && r.n2v.length) || (r.h2v && r.h2v.length));
  if (hasBreak || brk2 || after > 0) {
    var mathInner = '';
    if (hasBreak) {
      mathInner += '<div style="font-size:11px;font-weight:800;color:#94A3B8;letter-spacing:.3px;margin:2px 0 4px">ОДООГИЙН ҮНЭЛГЭЭ</div>' +
        '<div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:11px;padding:10px 13px">' +
        '<div style="display:flex;font-size:10.5px;font-weight:800;color:#94A3B8;letter-spacing:.3px;padding-bottom:4px;border-bottom:1px solid #E2E8F0">' +
        '<span style="flex:0 0 118px">ҮЗҮҮЛЭЛТ</span><span style="flex:1">ГИШҮҮДИЙН ОНОО</span><span style="flex:0 0 46px;text-align:right">ДУНДАЖ</span></div>' +
        riskScoreChips(r.pv, r.p, 'Магадлал (P)') +
        riskScoreChips(r.nv, r.n, 'Ослын үр дагавар (N)') +
        riskScoreChips(r.hv, r.h, 'Гишүүдийн санал (H)') +
        (r.rOk
          ? '<div style="border-top:1px solid #E2E8F0;padding-top:8px;margin-top:4px;text-align:center;font-size:13px;color:#334155">' +
            '<b>' + r.p + '</b> × <b>' + r.n + '</b> × <b>' + r.h + '</b> = ' +
            '<b style="color:' + L.color + ';font-size:15px">' + sc + '</b></div>'
          : '') + '</div>';
    }
    if (brk2) {
      mathInner += '<div style="font-size:11px;font-weight:800;color:#94A3B8;letter-spacing:.3px;margin:11px 0 4px">АРГА ХЭМЖЭЭНИЙ ДАРААХ ҮНЭЛГЭЭ</div>' +
        '<div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:11px;padding:10px 13px">' +
        riskScoreChips(r.p2v, r.p2, 'Магадлал (P)') +
        riskScoreChips(r.n2v, r.n2, 'Ослын үр дагавар (N)') +
        riskScoreChips(r.h2v, r.h2, 'Гишүүдийн санал (H)') + '</div>';
    }
    /* Нэг мөрт хураангуй — дэлгэрэнгүйг дарж нээнэ */
    var oneLine = un ? 'Оноо тавигдаагүй' :
      ((r.rOk ? r.p + ' × ' + r.n + ' × ' + r.h + ' = ' : 'R = ') +
       '<b style="color:' + L.color + '">' + sc + '</b>' + ' · ' + esc(L.name) +
       (after > 0 ? ' &nbsp;→&nbsp; арга хэмжээний дараа <b style="color:' + LA.color + '">' + after + '</b>' +
         (drop > 0 ? ' (' + drop + '%-иар буурна)' : '') : ''));
    H += '<details class="card" style="padding:13px 16px;margin-bottom:12px">' +
      '<summary style="cursor:pointer;list-style:none;display:flex;align-items:center;gap:9px">' +
      '<i class="ti ti-chevron-right" style="font-size:15px;color:#94A3B8"></i>' +
      '<span style="flex:1;min-width:0"><span style="display:block;font-size:11px;font-weight:800;color:#94A3B8;letter-spacing:.4px">' +
      'ҮНЭЛГЭЭ ЯАЖ ГАРСАН · R = P × N × H</span>' +
      '<span style="display:block;font-size:13px;color:#334155;margin-top:2px">' + oneLine + '</span></span></summary>' +
      '<div style="margin-top:11px">' + mathInner + '</div></details>';
  }
  H += '</div>';   /* ← зүүн багана дуусав */

  /* ── БАРУУН: юу хийж сэргийлэх вэ ──
  ...
  /* ── Бүртгэлийн мэдээлэл ── */
  var meta = [];
  if (r.createdAt) meta.push('Оруулсан: ' + String(r.createdAt).slice(0, 10));
  if (r.createdBy) meta.push(r.createdBy);
  if (meta.length) {
    H += '<div style="padding-top:9px;border-top:1px solid #F1F5F9;font-size:11px;color:#CBD5E1">' +
      esc(meta.join(' · ')) + '</div>';
  }
  return H;
}
...
function riskWire(sec, redraw) {
  ...
  sec.addEventListener('click', function (ev) {
    ...
    /* 5×5 матрицын нүд — тэр магадлал/үр дагаврын эрсдэлүүдийг шүүнэ */
    var mxb = ev.target.closest('[data-risk-mx]');
    if (mxb) {
      var mk = mxb.getAttribute('data-risk-mx');
      RISK_FILTER.mx = (RISK_FILTER.mx === mk) ? '' : mk;
    ...
    if (cr) { RISK_FILTER = { level: '', q: '', cell: 0, dept: '', mx: '' }; redraw(); return; }
...
function renderHazards() {
  ...
  if (!isAdmin() && !isDeptHead() && !list.length) {
    ...
    var k = riskKeyForEmp(myDept, myPos);
    if (k) { renderRiskDept(sec, k); return; }
  }
  ...
  if (!list.length) {
    ...
    if (RISK_TAB === 'mea') {
      ...
      H += admin ? riskDashHTML(list, sub) + riskAdminSectionsHTML(list)
  ...
  if (RISK_TAB === 'mea') {
    ...
    if (box) {
      MEA_RERENDER = renderHazards;
      if (isAdmin() || isDeptHead()) renderMeasureAdminPage(box, true);
      else renderMeasurePage(box, true);
    }
  }
...
/* ══════════════════════════════════════════════════════════════════════
   ЭРСДЭЛИЙН ЗАГВАР (Excel) — АЛДААГҮЙ ОРУУЛАХ
   ----------------------------------------------------------------------
   Загварыг СИСТЕМЭЭС ӨӨРӨӨС НЬ үүсгэнэ: «Лавлах» хуудсанд таны бүртгэл
   дэх алба, албан тушаалын ЯГ ЖАГСААЛТ орно. Тэндээс хуулж бөглөвөл
   нэр таарахгүй байх асуудал үүсэхгүй.
   R = P × N × H нь ТОМЬЁОГООР бодогдоно — гараар бодох шаардлагагүй.
   ══════════════════════════════════════════════════════════════════════ */
var RISK_TPL_COLS = [
  'Алба', 'Албан тушаал', 'Үйл ажиллагаа', 'Болзошгүй аюул', 'Аюулын байршил', 'Шалтгаан',
  'P магадлал (1-5)', 'N үр дагавар (1-5)', 'H санал (1-5)', 'R эрсдэлийн зэрэг',
  'Авах арга хэмжээ', 'Хэн хяналт тавих', 'Хэзээ',
  'P2 (дараа)', 'N2 (дараа)', 'H2 (дараа)', 'R2 үлдэгдэл эрсдэл'
];

function actionRiskTemplate() {
  riskLoadXlsx(function (ok) {
    if (!ok) { toast('Excel үүсгэгч ачаалагдсангүй', 'error'); return; }
    var emps = DB.employees || [];
    var lockDept = isDeptHead() ? ((SESSION && SESSION.dept) || '') : '';

    /* Системд БОДИТООР байгаа алба, албан тушаал */
    var depts = {}, poss = {};
    emps.forEach(function (e) {
      if (lockDept && e.dept !== lockDept) return;
      if (e.dept) depts[e.dept] = (depts[e.dept] || 0) + 1;
      var p = e.role || e.pos;
      if (p) poss[p] = (poss[p] || 0) + 1;
    });
    var dNames = Object.keys(depts).sort();
    var pNames = Object.keys(poss).sort();
    var d0 = dNames[0] || 'Алба', p0 = pNames[0] || '';

    var wb = XLSX.utils.book_new();

    /* ── 1. ЗААВАР ── */
    var guide = [
      ['ЭРСДЭЛИЙН ҮНЭЛГЭЭ ОРУУЛАХ ЗААВАР'], [],
      ['1', 'Эрсдэл гэсэн хуудсыг бөглөнө. Мөр бүр = НЭГ эрсдэл.'],
      ['2', 'Алба ба Албан тушаалыг «Лавлах» хуудаснаас ХУУЛЖ тавина (гараар бүү бич).'],
      ['3', 'Албан тушаалыг хоосон орхивол тэр албаны БҮХ ажилтанд харагдана.'],
      ['4', 'Нэг эрсдэл олон албан тушаалд хамаарвал таслалаар зааглана. Ж: Оператор, Технологич'],
      ['5', 'P, N, H-д зөвхөн 1-5 хүртэлх бүхэл тоо бичнэ. R өөрөө бодогдоно.'],
      ['6', 'P2, N2, H2 = арга хэмжээ авсны ДАРААХ үнэлгээ (заавал биш).'],
      [], ['АРГАЧЛАЛ: R = P × N × H'], [],
      ['P', 'Аюул үүсэх магадлал', '1 тохиолдлын · 2 бага · 3 магадлалтай · 4 бүрэн · 5 тогтмол'],
    ...
    wsG['!cols'] = [{ wch: 6 }, { wch: 30 }, { wch: 70 }];
    XLSX.utils.book_append_sheet(wb, wsG, 'Заавар');

    /* ── 2. ЭРСДЭЛ (бөглөх хуудас) ── */
    var rows = [RISK_TPL_COLS];
    var ex = function (dept, pos, act, hz, loc, cause, p, n, h, meas, who, when) {
      return [dept, pos, act, hz, loc, cause, p, n, h, '', meas, who, when, '', '', '', ''];
    };
    rows.push(ex(d0, p0, 'Ачаа өргөх', 'Нурууны гэмтэл', 'Агуулах',
      'Гараар хүнд ачаа өргөх', 3, 3, 2, 'Гараар ачаа өргөх сургалт', 'ХАБЭА', 'Тухай бүр'));
    rows.push(ex(d0, '', 'Ажилдаа ирж буцах', 'Замын осол', 'Ажилдаа ирэх зам',
      'Замын хөдөлгөөний дүрэм зөрчих', 2, 2, 1, 'Давтан зааварчилгаа', 'ХАБЭА', 'Улирал бүр'));
    for (var i = 0; i < 60; i++) rows.push(['', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '']);

    var ws = XLSX.utils.aoa_to_sheet(rows);
    /* R ба R2-г ТОМЬЁО болгоно — гараар бодох шаардлагагүй, алдаа гарахгүй */
    for (var r = 2; r <= rows.length; r++) {
      ws['J' + r] = { t: 'n', f: 'IF(COUNT(G' + r + ':I' + r + ')=3,G' + r + '*H' + r + '*I' + r + ',"")' };
      ws['Q' + r] = { t: 'n', f: 'IF(COUNT(N' + r + ':P' + r + ')=3,N' + r + '*O' + r + '*P' + r + ',"")' };
    }
    ws['!cols'] = [{ wch: 26 }, { wch: 24 }, { wch: 26 }, { wch: 40 }, { wch: 20 }, { wch: 34 },
      { wch: 15 }, { wch: 16 }, { wch: 14 }, { wch: 16 }, { wch: 36 }, { wch: 18 }, { wch: 14 },
      { wch: 11 }, { wch: 11 }, { wch: 11 }, { wch: 17 }];
    XLSX.utils.book_append_sheet(wb, ws, 'Эрсдэл');

    /* ── 3. ЛАВЛАХ (системийн ЯГ жагсаалт) ── */
    var ref = [['СИСТЕМД БҮРТГЭЛТЭЙ АЛБА', 'Ажилтан', '', 'СИСТЕМД БҮРТГЭЛТЭЙ АЛБАН ТУШААЛ', 'Ажилтан']];
    var maxLen = Math.max(dNames.length, pNames.length);
    for (var k = 0; k < maxLen; k++) {
      ref.push([
        dNames[k] || '', dNames[k] ? depts[dNames[k]] : '', '',
        pNames[k] || '', pNames[k] ? poss[pNames[k]] : ''
      ]);
    }
    var wsR = XLSX.utils.aoa_to_sheet(ref);
    wsR['!cols'] = [{ wch: 34 }, { wch: 9 }, { wch: 3 }, { wch: 34 }, { wch: 9 }];
    XLSX.utils.book_append_sheet(wb, wsR, 'Лавлах');

    var stamp = _ymd(new Date());
    XLSX.writeFile(wb, 'Эрсдэлийн-үнэлгээ-загвар-' + stamp + '.xlsx');
    toast('Загвар татагдлаа — «Лавлах» хуудсаас алба, албан тушаалаа хуулна уу', 'success');
  });
}
...
/* ZIP уншигч — Google Drive-аас татсан архивыг задлахгүйгээр шууд оруулахад */
function riskLoadZip(cb) {
  if (typeof JSZip !== 'undefined') { cb(true); return; }
  var s = document.createElement('script');
...
/* ZIP доторх Excel файлуудыг { name, buf } болгож гаргана */
function riskUnzip(file, cb) {
  riskLoadZip(function (ok) {
    if (!ok) { cb(null, 'ZIP уншигч ачаалагдсангүй'); return; }
    ...
    rd.onload = function (e) {
      JSZip.loadAsync(e.target.result).then(function (zip) {
        ...
        zip.forEach(function (path, entry) {
          if (entry.dir) return;
          if (!/\.(xlsx|xls|csv)$/i.test(path)) return;
          if (/(^|\/)(__MACOSX|~\$)/.test(path)) return;   // системийн хог файл
          jobs.push(entry.async('arraybuffer').then(function (buf) {
            return { name: path, buf: buf };
          }));
        });
        if (!jobs.length) { cb([], ''); return; }
        Promise.all(jobs).then(function (arr) { cb(arr, ''); })
          .catch(function (er) { cb(null, er.message || 'задалж чадсангүй'); });
      }).catch(function (er) { cb(null, er.message || 'ZIP биш байна'); });
    };
    rd.onerror = function () { cb(null, 'файл уншиж чадсангүй'); };
...
function riskMapColsReal(head, sub) {
  ...
  /* ⚠ N, H, P тус бүр ГУРВАН багана эзэлдэг — эрсдэлийн үнэлгээнд оролцсон
     3 гишүүн тус тусдаа оноо өгдөг. Толгой нь нийлүүлсэн нүд тул зөвхөн
     эхний баганад бичигдсэн байдаг. Эрсдэлийн зэрэг R нь тэдгээрийн
     ДУНДЖИЙН үржвэр:  R = P̄ × N̄ × H̄
     Эхний баганыг л уншвал нэг гишүүний оноо гарч, R-тэй таарахгүй. */
  var labelled = [];
  h.forEach(function (x, i) { if (x) labelled.push(i); });
  labelled.sort(function (a, b) { return a - b; });
  var spanEnd = function (start) {
    for (var i = 0; i < labelled.length; i++) if (labelled[i] > start) return labelled[i] - 1;
    return h.length - 1;
  };
  var all = function (k) { var out = []; h.forEach(function (x, i) { if (x.indexOf(k) >= 0) out.push(i); }); return out; };
  ...
  return {
    ...
    n:           nI.length ? nI[0] : -1,
    h:           hI.length ? hI[0] : -1,
    p2:          pI.length > 1 ? pI[pI.length - 1] : -1,
    n2:          nI.length > 1 ? nI[nI.length - 1] : -1,
    h2:          hI.length > 1 ? hI[hI.length - 1] : -1,
    r:           rc.length ? rc[0] : -1,
...
/* Албан тушаалын товчлол / ижил утгатай нэрсийг нэг болгоно.
   • «ГЗ» бол Гүйцэтгэх захирал
   • «Оффис менежер» ба «захирлын туслах» — НЭГ ажилтан гүйцэтгэдэг тул
     нэг албан тушаал болгож нэгтгэнэ (эс бөгөөс нэг эрсдэл хоёр удаа орно) */
var RISK_POS_ALIAS = {
  'гз': 'Гүйцэтгэх захирал',
  'гүйцэтгэх захирал': 'Гүйцэтгэх захирал',
  'оффис менежер': 'Оффис менежер, захирлын туслах',
  'офис менежер': 'Оффис менежер, захирлын туслах',
  'захирлын туслах': 'Оффис менежер, захирлын туслах',
  'захирлын туслах, оффис менежер': 'Оффис менежер, захирлын туслах'
};
function riskPosAlias(t) {
  var k = String(t || '').replace(/\s+/g, ' ').trim().toLowerCase();
  return RISK_POS_ALIAS[k] || t;
}
...
function actionRiskFolder() {
  ...
  var node = elc('div', 'modal-info',
    ...
    'Эрсдэлийн үнэлгээний <b>фолдероо бүхэлд нь</b> эсвэл <b>ZIP архивыг</b> шууд сонгоно уу — задлах шаардлагагүй.<br>' +
  ...
  /* Аль ч эх сурвалжаас ирсэн { name, buf } жагсаалтыг нэг ижил замаар боловсруулна */
  var processBufs = function (items, st) {
    riskLoadXlsx(function (ok) {
      ...
      items.forEach(function (it) {
        var res;
        try {
          var wb = XLSX.read(new Uint8Array(it.buf), { type: 'array' });
          res = riskParseSheet(wb, it.name, depts, lockDept);
        } catch (er) { res = { rows: [], dept: '', err: (er && er.message) || 'уншиж чадсангүй' }; }
        report.push({ name: it.name.split('/').pop(), dept: res.dept, n: res.rows.length, err: res.err });
        res.rows.forEach(function (r) { staged.push(r); });
      });
      finish(report, staged, st);
    });
  };
  ...
  var handle = function (files) {
    ...
    var xl = arr.filter(function (f) { return /\.(xlsx|xls|csv)$/i.test(f.name); });

    if (!zips.length && !xl.length) {
      st.innerHTML = '<div style="color:#DC2626;font-size:13px">Excel эсвэл ZIP файл олдсонгүй.</div>'; return;
    }
    st.innerHTML = '<div style="font-size:13px;color:#64748B">Уншиж байна…</div>';

    var items = [], pending = zips.length + xl.length, zipErr = '';
    var step = function () {
      if (--pending > 0) return;
      if (!items.length) {
        st.innerHTML = '<div style="color:#DC2626;font-size:13px">Excel файл олдсонгүй' +
          (zipErr ? ' (' + esc(zipErr) + ')' : '') + '.</div>'; return;
      }
      processBufs(items, st);
    };

    zips.forEach(function (z) {
      riskUnzip(z, function (list, err) {
        if (err) zipErr = err;
        (list || []).forEach(function (x) { items.push({ name: z.name.replace(/\.zip$/i, '') + '/' + x.name, buf: x.buf }); });
        step();
      });
    });
    xl.forEach(function (f) {
      var rd = new FileReader();
      rd.onload = function (e) { items.push({ name: f.webkitRelativePath || f.name, buf: e.target.result }); step(); };
...
function actionRiskTemplateImport() {
  if (!isAdmin() && !isDeptHead()) { toast('Зөвхөн админ/туслах админ оруулна', 'error'); return; }
  var lockDept = isDeptHead() ? ((SESSION && SESSION.dept) || '') : '';
  var emps = DB.employees || [];
  var sysDepts = {}, sysPos = {};
  emps.forEach(function (e) {
    if (e.dept) sysDepts[String(e.dept).trim().toLowerCase()] = e.dept;
    var p = e.role || e.pos;
    if (p) sysPos[String(p).trim().toLowerCase()] = p;
  });

  var node = elc('div', 'modal-info',
    '<div style="font-size:13.5px;color:#475569;line-height:1.7;margin-bottom:14px">' +
    'Загвараар бөглөсөн Excel файлаа сонгоно уу. Систем мөр бүрийг шалгаж, ' +
    '<b>алдаатай мөрийг тус бүрд нь заана</b> — алдаатай бол юу ч хадгалагдахгүй.</div>' +
    '<label class="btn btn-primary" style="cursor:pointer;display:inline-flex;align-items:center;gap:7px">' +
    '<input type="file" id="riskTplFile" accept=".xlsx,.xls" style="display:none">' +
    '<i class="ti ti-upload"></i> Бөглөсөн файлаа сонгох</label>' +
    '<div id="riskTplSt" style="margin-top:16px"></div>');
  buildModal('Загвараар оруулах', node, { width: '660px' });

  document.getElementById('riskTplFile').addEventListener('change', function () {
    var f = this.files && this.files[0]; if (!f) return;
    var st = document.getElementById('riskTplSt');
    st.innerHTML = '<div style="font-size:13px;color:#64748B">Шалгаж байна…</div>';
    riskLoadXlsx(function (ok) {
      if (!ok) { st.innerHTML = '<div style="color:#DC2626;font-size:13px">Excel уншигч ачаалагдсангүй.</div>'; return; }
      var rd = new FileReader();
      rd.onload = function (e) {
        ...
        try {
          ...
          var sh = wb.Sheets['Эрсдэл'] || wb.Sheets[wb.SheetNames[0]];
          aoa = XLSX.utils.sheet_to_json(sh, { header: 1, defval: '' });
        } catch (er) { st.innerHTML = '<div style="color:#DC2626;font-size:13px">Файл уншиж чадсангүй.</div>'; return; }

        var head = (aoa[0] || []).map(function (x) { return String(x || '').trim(); });
        var idx = {}; RISK_TPL_COLS.forEach(function (c) { idx[c] = head.indexOf(c); });
        if (idx['Болзошгүй аюул'] < 0 || idx['Алба'] < 0) {
          st.innerHTML = '<div style="color:#DC2626;font-size:13px;line-height:1.6">' +
            'Энэ файл загварын бүтэцтэй биш байна. «Загвар татах» товчоор загварыг аваад түүн дээрээ бөглөнө үү.</div>';
          return;
        }
        var g = function (row, c) { var i = idx[c]; return i < 0 ? '' : String(row[i] == null ? '' : row[i]).replace(/\s+/g, ' ').trim(); };
        var errs = [], good = [];
        for (var r = 1; r < aoa.length; r++) {
          var row = aoa[r] || [];
          var hz = g(row, 'Болзошгүй аюул');
          var dept = lockDept || g(row, 'Алба');
          if (!hz && !g(row, 'Алба')) continue;                       // хоосон мөр — алгасна
          var line = r + 1, bad = [];
          if (!hz) bad.push('«Болзошгүй аюул» хоосон');
          if (!dept) bad.push('«Алба» хоосон');
          else if (!sysDepts[dept.toLowerCase()]) bad.push('«' + dept + '» гэсэн алба системд алга');
          var posRaw = g(row, 'Албан тушаал');
          var posList = posRaw ? posRaw.split(/\s*,\s*/).filter(Boolean) : [];
          posList.forEach(function (p) {
            if (!sysPos[p.toLowerCase()]) bad.push('«' + p + '» гэсэн албан тушаал системд алга');
          });
          var P = _f(g(row, 'P магадлал (1-5)'), 0), N = _f(g(row, 'N үр дагавар (1-5)'), 0), H = _f(g(row, 'H санал (1-5)'), 0);
          [['P', P], ['N', N], ['H', H]].forEach(function (x) {
            if (!(x[1] >= 1 && x[1] <= 5)) bad.push(x[0] + ' нь 1-5 хооронд байх ёстой (одоо: ' + (x[1] || 'хоосон') + ')');
          });
          if (bad.length) { errs.push({ line: line, msg: bad.join(' · ') }); continue; }
          var P2 = _f(g(row, 'P2 (дараа)'), 0), N2 = _f(g(row, 'N2 (дараа)'), 0), H2 = _f(g(row, 'H2 (дараа)'), 0);
          good.push({
            dept: sysDepts[dept.toLowerCase()],
            positions: posList.map(function (p) { return sysPos[p.toLowerCase()]; }),
            position: posList.length === 1 ? sysPos[posList[0].toLowerCase()] : '',
            process: g(row, 'Үйл ажиллагаа'),
            hazard: hz, location: g(row, 'Аюулын байршил'), cause: g(row, 'Шалтгаан'),
            prob: P, sev: N, hcoef: H, r: P * N * H,
            rAfter: (P2 && N2 && H2) ? P2 * N2 * H2 : 0,
            actions: g(row, 'Авах арга хэмжээ'), responsible: g(row, 'Хэн хяналт тавих'),
            due: g(row, 'Хэзээ'), empIds: []
          });
        }

        if (errs.length) {
          st.innerHTML =
            '<div style="background:#FEF2F2;border:1px solid #FECACA;border-radius:12px;padding:12px 14px;margin-bottom:10px">' +
            '<div style="font-size:18px;font-weight:900;color:#DC2626">' + errs.length + ' мөрөнд алдаа байна</div>' +
            '<div style="font-size:12.5px;color:#991B1B">Засаад дахин оруулна уу. Одоохондоо юу ч хадгалагдаагүй.</div></div>' +
            '<div style="max-height:260px;overflow:auto">' +
            errs.slice(0, 40).map(function (x) {
              return '<div style="display:flex;gap:10px;padding:7px 0;border-bottom:1px solid #F1F5F9">' +
                '<span style="flex:0 0 62px;font-size:12px;font-weight:800;color:#DC2626">мөр ' + x.line + '</span>' +
                '<span style="flex:1;font-size:12.5px;color:#334155;line-height:1.5">' + esc(x.msg) + '</span></div>';
            }).join('') + '</div>' +
            (errs.length > 40 ? '<div style="font-size:12px;color:#94A3B8;margin-top:8px">…нийт ' + errs.length + '</div>' : '');
          return;
        }
        if (!good.length) { st.innerHTML = '<div style="color:#DC2626;font-size:13px">Бөглөсөн мөр олдсонгүй.</div>'; return; }

        st.innerHTML =
          '<div style="background:#F0FDF4;border:1px solid #BBF7D0;border-radius:12px;padding:12px 14px;margin-bottom:12px">' +
          '<div style="font-size:20px;font-weight:900;color:#16A34A;font-family:\'Bricolage Grotesque\',sans-serif">' + good.length + ' эрсдэл</div>' +
          '<div style="font-size:12.5px;color:#166534">Бүх мөр зөв — алдаа алга ✓</div></div>' +
          '<button class="btn btn-primary" id="riskTplGo" style="width:100%">' +
          '<i class="ti ti-database-import"></i> ' + good.length + ' эрсдлийг системд оруулах</button>';
        document.getElementById('riskTplGo').addEventListener('click', function () {
          var now = Date.now();
...
/* ============ Даалгавар ============ */
/* ══════════════════════════════════════════════════════════════════════
   ДААЛГАВРЫН БИЕ — эрсдэлээс үүссэн бол ТОДОРХОЙ товьёог харуулна:
   «Та ямар аюулыг, юу хийж, хэзээ хүртэл бууруулах вэ».
   ⚠ Эрсдэл ачаалагдаагүй бол анхны тайлбарыг ХЭВЭЭР харуулна (юу ч алдагдахгүй).
   ══════════════════════════════════════════════════════════════════════ */
function taskRiskOf(x) {
  if (!x || !x.riskId) return null;
  return (DB.risks || []).filter(function (r) { return String(r.id) === String(x.riskId); })[0] || null;
}
function taskBodyHTML(x) {
  var r = taskRiskOf(x);
  if (!r) {
    return x.desc ? '<div style="font-size:13px;color:#64748B;margin-top:3px;white-space:pre-wrap">' + esc(x.desc) + '</div>' : '';
  }
  var L = riskLevel(r), sc = riskScore(r);
  var after = riskScoreAfter(r);
  var line = function (ic, t, v) {
    if (!v) return '';
    return '<div style="display:flex;gap:8px;margin-top:5px">' +
      '<span style="flex:0 0 16px;font-size:12px;line-height:1.5">' + ic + '</span>' +
      '<span style="flex:1;min-width:0;font-size:12.5px;color:#475569;line-height:1.55">' +
      '<b style="color:#334155">' + t + ':</b> ' + esc(String(v)) + '</span></div>';
  };
  return '<div style="margin-top:9px;border:1.5px solid ' + L.bd + ';border-radius:11px;overflow:hidden">' +
    /* Аль аюулыг бууруулах вэ */
    '<div style="background:' + L.bg + ';padding:10px 12px;display:flex;align-items:center;gap:10px">' +
    '<span style="flex:0 0 auto;background:' + L.color + ';color:#fff;border-radius:7px;padding:3px 8px;font-size:11px;font-weight:800">' +
    L.code + ' · ' + sc + '</span>' +
    '<span style="flex:1;min-width:0;font-size:13px;font-weight:700;color:#1E293B;line-height:1.4">' +
    esc(r.hazard || '') + '</span></div>' +
    '<div style="padding:9px 12px 11px;background:#fff">' +
    '<div style="font-size:11px;font-weight:800;color:#94A3B8;letter-spacing:.3px">ТА ЭНЭ АЮУЛЫГ БУУРУУЛНА</div>' +
    line('⚙️', 'Ажиллагаа', r.process) +
    line('📍', 'Байршил', r.location) +
    line('❓', 'Шалтгаан', r.cause) +
    line('🛡️', 'Авах арга хэмжээ', r.actions) +
    line('👤', 'Хяналт тавих', r.responsible) +
    line('📅', 'Хугацаа', x.dueDate || r.due) +
    (after > 0 && sc > after
      ? '<div style="margin-top:8px;background:#F0FDF4;border:1px solid #BBF7D0;border-radius:8px;padding:7px 10px;font-size:12px;color:#15803D">' +
        '🎯 Зорилт: эрсдлийг <b>' + sc + '</b> → <b>' + after + '</b> болгож бууруулах' +
        ' (<b>' + Math.round((1 - after / sc) * 100) + '%</b>)</div>'
      : '') +
    '<button class="btn btn-sm" data-risk-open="' + esc(r.id) + '" style="margin-top:9px;background:#F1F5F9;color:#475569;border-color:#E2E8F0">' +
    '<i class="ti ti-info-circle"></i> Эрсдэлийн дэлгэрэнгүй</button>' +
    '</div></div>';
}
...
/* ⭐ Арга хэмжээний хуудсыг ЭРСДЭЛИЙН ҮНЭЛГЭЭ цэсний таб дотор зурна.
   Дата ачаалагдмагц аль хуудсыг дахин зурахыг энэ заана (renderHazards). */
var MEA_RERENDER = null;
function meaRedraw() {
  if (typeof MEA_RERENDER === 'function') { MEA_RERENDER(); return; }
  try { renderHazards(); } catch (e) {}
}

/* Ажилтны «Арга хэмжээний биелэлт» — embed=true бол гарчиггүйгээр таб дотор */
function renderMeasurePage(sec, embed) {
  if (!sec) return;
  var me = null; try { me = myEmp(); } catch (e) {}
  if (!me) {
  ...
  /* ⚠ taskBadge нь «Даалгавар» цэсний тоолуур — арга хэмжээгээр дарж
     бичихгүй. Арга хэмжээний хугацаа хэтэрснийг эрсдэлийн цэсний тугаар
     (riskTabsHTML) ба нүүр хуудсаар сануулна. */
...
function renderTasks() {
  ...
  sec.style.padding = '';
  /* ⚠ «Даалгавар» бол ТУСДАА цэс — эрсдэлээс гарсан арга хэмжээтэй хольж
     болохгүй. Арга хэмжээ нь ЭРСДЭЛИЙН ҮНЭЛГЭЭ цэсэн дээрээ (таб болж) ба
     эрсдэл бүрийн дэлгэрэнгүй дотроо гүйцэтгэгддэг. Энэ цэс нь удирдлага
     ажилтанд гараар даалгавар өгч, хянаж, үнэлдэг өөр үйл ажиллагаа. */
  DB.tasks = DB.tasks || [];
